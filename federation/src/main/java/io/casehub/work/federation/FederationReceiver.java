...
public class FederationReceiver {
    ...
    public void onEvent(String cloudEventJson, String signature, byte[] hmacSecret) {
        ...
        JsonNode ce;
        try {
            ce = objectMapper.readTree(cloudEventJson);
        } catch (Exception e) {
            throw new IllegalArgumentException("Invalid CloudEvent JSON", e);
        }
        ...
        if (tenancyId == null || tenancyId.isEmpty()) {
            throw new IllegalArgumentException("Missing tenancyid extension in federation CloudEvent");
        }

        tenantContextRunner.runInTenantContext(tenancyId, () ->
                                                                  processInTenantContext(type, sourceServiceId, tenancyId, incomingVersion, data));
    }

    @Transactional
    void processInTenantContext(String type, String sourceServiceId,
                                String tenancyId, long incomingVersion, JsonNode data) {
        ...
        String namespacedCallerRef = projection.callerRef() != null
                                     ...
                                     : null;
        projection = projection.toBuilder()
                               .callerRef(namespacedCallerRef)
                               .tenancyId(tenancyId)
                               .build();
