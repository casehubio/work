...
public class FederationProxy {
    ...
    private volatile WorkItemClient client;

    private WorkItemClient client() {
        if (client == null) {
            client = new WorkItemClient(Duration.ofSeconds(config.proxyTimeoutSeconds()));
        }
        return client;
    }

    public WorkItem claim(WorkItem shadow, String claimantId) {
        String ownerUrl = resolveOwnerUrl(shadow);
    ...
    private String resolveOwnerUrl(WorkItem shadow) {
        ...
        return subscriptions.stream()
                ...
                .map(s -> s.callbackUrl.replace("/federation/events", ""))
                .findFirst()
