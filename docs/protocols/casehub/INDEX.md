...
| [queue-filter-scope-management-only.md](queue-filter-scope-management-only.md) | Filter/queue scope is management metadata — findActive()/scanAll() must not filter by scope | WorkItemFilterStore, QueueViewStore, any future queues store query |
