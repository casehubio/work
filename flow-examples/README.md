...
mvn install -DskipTests -pl runtime,deployment,testing,work-flow
```

### Start in dev mode

```bash
JAVA_HOME=$(/usr/libexec/java_home -v 26) mvn quarkus:dev -pl quarkus-work-flow-examples
```
...
```bash
JAVA_HOME=$(/usr/libexec/java_home -v 26) mvn test -pl quarkus-work-flow-examples
```
...
needs to suspend for a human decision. See `quarkus-work-flow/README.md` for the full API reference.
